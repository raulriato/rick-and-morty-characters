<template>
  <div class="wrapper" @mouseover="() => { shown = true; hidden = true}" @mouseout="() => { shown = false; hidden = false}">
    <img :src="character.image" alt="loading..." />
    <div class="gradient" />
    <CTitle :title="character.name" :hidden="hidden" />
    <CInfo :name="character.name" :status="character.status" :type="character.type" :species="character.species" :gender="character.gender" :shown="shown" />
  </div>
</template>
...
.wrapper {
  ...
  .gradient {
    ...
    &:hover {
      background: linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, rgba(0, 3, 51, 0.70) 40.19%, #333 98%);
    }
